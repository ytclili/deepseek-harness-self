...
Use a Harness 0.1.7 source checkout at `/mnt/sata4-2/www/code/deepseek-harness-self` containing the updated `enterprise-auth`, `enterprise-tools`, and `web-portal` plugins. `build-images.sh` automatically runs `prepare-runtime.sh`: a disposable source copy is installed and built in the Linux Node 24 container, then each plugin runs setup, check, and test. The portal client runs `npm ci`; its test command builds the host and client before testing. A missing portal build is therefore handled by the preparation step. The active source checkout and administrator service remain untouched.
