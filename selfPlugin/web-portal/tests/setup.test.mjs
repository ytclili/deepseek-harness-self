...
const packages = {
  'dsh-enterprise-auth': ['selfPlugin/enterprise-auth', '0.1.0'],
  ...
  '@deepseek-ai/dsh-host-webserver': ['packages/host/webserver', '0.1.7-alpha.1'],
  '@deepseek-ai/dsh-client-connection': ['packages/client/connection', '0.1.7-alpha.1'],
  '@deepseek-ai/dsh-web-frontend': ['apps/web', '0.1.7-alpha.1'],
  '@types/node': ['node_modules/@types/node', '22.20.0'],
  typescript: ['node_modules/typescript', '6.0.3'],
}

async function fixture(t, mismatch) {
  const directory = await mkdtemp(join(tmpdir(), 'portal-setup-'))
  t.after(() => rm(directory, { recursive: true, force: true }))
  const harness = join(directory, 'harness')
  const plugin = join(harness, 'selfPlugin/web-portal')
